...
if (isset($_POST["import"])) {
  ...
  $cart = ambil_data("SELECT * FROM cart");
  $produk = ["produk", "produk_sofa", "produk_lemari_dapur", "produk_meja", "produk_rak_tv", "produk_kasur", "produk_lemari"];
  $laporan = [];
  foreach ($produk as $prd) {
    array_push($laporan, ambil_data("SELECT * FROM $prd"));
  }
  foreach ($laporan as $lpn) {
    foreach ($lpn as $l) {
      foreach ($cart as $c) {
        if ($c["kode_produk"] == $l["kode_produk"]) {
          foreach ($produk as $p) {
            $cari_tabel_produk = ambil_data("SELECT * FROM $p");
            foreach ($cari_tabel_produk as $ctp) {
              if (in_array($c["kode_produk"], $ctp)) {
                $qty = $c["qty"];
                $kode = $c["kode_produk"];
                mysqli_query($conn, "UPDATE $p
                                              SET stok = stok - $qty
...
$dataselect1 = mysqli_query($conn, "SELECT * FROM produk_sofa");
$dataselect2 = mysqli_query($conn, "SELECT * FROM produk_lemari_dapur");
$dataselect3 = mysqli_query($conn, "SELECT * FROM produk_meja");
$dataselect4 = mysqli_query($conn, "SELECT * FROM produk_rak_tv");
$dataselect5 = mysqli_query($conn, "SELECT * FROM produk_kasur");
$dataselect6 = mysqli_query($conn, "SELECT * FROM produk_lemari");
...
<form method="post">
  <div class="row">
    ...
    foreach ($laporan as $lpn) {
      foreach ($lpn as $l) {
    ?>
        <input type="hidden" id="stok_barang" value="<?= $l["stok"]; ?>" data-kode="<?= $l["kode_produk"]; ?>">
    <?php }
    } ?>
        ...
        <datalist id="datalist1">
          ...
          if (mysqli_num_rows($dataselect,)) { ?>
            ...
            <?php if (mysqli_num_rows($dataselect1,)) { ?>
              <?php while ($row_brg = mysqli_fetch_array($dataselect1)) { ?>
              <option value="<?php echo $row_brg["kode_produk"] ?>"> <?php echo $row_brg["kode_produk"] ?>
                <input type="hidden" id="stok_barang" value="<?= $row_brg["stok"]; ?>">
              <?php
                $jsArray .= "nama_produk['" . $row_brg['kode_produk'] . "'] = {nama_produk:'" . addslashes($row_brg['nama_produk']) . "'};";
            ...
            <?php } ?>
            <?php if (mysqli_num_rows($dataselect2,)) { ?>
              <?php while ($row_brg = mysqli_fetch_array($dataselect2)) { ?>
              <option value="<?php echo $row_brg["kode_produk"] ?>"> <?php echo $row_brg["kode_produk"] ?>
                <input type="hidden" id="stok_barang" value="<?= $row_brg["stok"]; ?>">
              <?php
                $jsArray .= "nama_produk['" . $row_brg['kode_produk'] . "'] = {nama_produk:'" . addslashes($row_brg['nama_produk']) . "'};";
            ...
            <?php } ?>
            <?php if (mysqli_num_rows($dataselect3,)) { ?>
              <?php while ($row_brg = mysqli_fetch_array($dataselect3)) { ?>
              <option value="<?php echo $row_brg["kode_produk"] ?>"> <?php echo $row_brg["kode_produk"] ?>
                <input type="hidden" id="stok_barang" value="<?= $row_brg["stok"]; ?>">
              <?php
                $jsArray .= "nama_produk['" . $row_brg['kode_produk'] . "'] = {nama_produk:'" . addslashes($row_brg['nama_produk']) . "'};";
            ...
            <?php } ?>
            <?php if (mysqli_num_rows($dataselect4,)) { ?>
              <?php while ($row_brg = mysqli_fetch_array($dataselect4)) { ?>
              <option value="<?php echo $row_brg["kode_produk"] ?>"> <?php echo $row_brg["kode_produk"] ?>
                <input type="hidden" id="stok_barang" value="<?= $row_brg["stok"]; ?>">
              <?php
                $jsArray .= "nama_produk['" . $row_brg['kode_produk'] . "'] = {nama_produk:'" . addslashes($row_brg['nama_produk']) . "'};";
            ...
            <?php } ?>
            <?php if (mysqli_num_rows($dataselect5,)) { ?>
              <?php while ($row_brg = mysqli_fetch_array($dataselect5)) { ?>
              <option value="<?php echo $row_brg["kode_produk"] ?>"> <?php echo $row_brg["kode_produk"] ?>
                <input type="hidden" id="stok_barang" value="<?= $row_brg["stok"]; ?>">
              <?php
                $jsArray .= "nama_produk['" . $row_brg['kode_produk'] . "'] = {nama_produk:'" . addslashes($row_brg['nama_produk']) . "'};";
            ...
            <?php } ?>
            <?php if (mysqli_num_rows($dataselect6,)) { ?>
              <?php while ($row_brg = mysqli_fetch_array($dataselect6)) { ?>
              <option value="<?php echo $row_brg["kode_produk"] ?>"> <?php echo $row_brg["kode_produk"] ?>
                <input type="hidden" id="stok_barang" value="<?= $row_brg["stok"]; ?>">
              <?php
                $jsArray .= "nama_produk['" . $row_brg['kode_produk'] . "'] = {nama_produk:'" . addslashes($row_brg['nama_produk']) . "'};";
    ...
    <div class="col-4 col-sm-4 col-md-4 col-lg-1 mb-3">
      ...
      <input type="number" name="Cqty" id="Iqty" min="1" onchange="InputSub()" placeholder="0" class="form-control form-control-sm" required>
    </div>
    <div class="col-sm-8 col-md-8 col-lg-3 mb-3">
      ...
      <div class="input-group">
        ...
        <div class="input-group-append">
          <button type="reset" class="btn btn-info btn-sm mr-2">Reset</button>
          <button type="submit" name="InputCart" class="btn btn-success btn-sm">Beli</button>
        </div>
      </div>
    </div>
    ...
    ?>
      <tr>
        ...
        <td><a class="btn btn-danger btn-xs" href="?hapus=<?php echo $d['idcart']; ?>">
            <i class="fas fa-trash-alt fa-sl mr-1"></i>Hapus</a>
        </td>
      </tr>
    <?php } ?>
  </tbody>
...
<div class="bg-light p-3" style="border-radius:0.25rem;">
  <div class="row gy-3 align-items-center row-home">
      ...
      <form method="post">
        ...
        <div class="row">
          ...
          <div class="col-8 col-sm-8 col-md-8 col-lg-9 mb-2">
            <input type="number" name="pembayaran" onchange="procesBayar()" class="form-control form-control-sm" id="pembayaran" placeholder="0" min="<?php echo ($itungtrans3) ?>" required>
          </div>
          ...
          <div class="col-sm-12 text-right">
            ...
            ?>
              ...
              <button type="submit" name="import" class="btn btn-info btn-sm px-3">
                <i class="fa fa-check mr-1"></i>Selesai</button>
            <?php } else { ?>
              <button class="btn btn-danger btn-sm px-3 mr-2" disabled>
                <i class="fa fa-trash-alt mr-1"></i>Batalkan Semua</button>
              <button class="btn btn-info btn-sm px-3" disabled>
                <i class="fa fa-check mr-1"></i>Selesai</button>
            <?php  } ?>
          </div>
        </div>
    ...
    <div class="col-md-4 col-lg-6 mb-2 text-warning text-right">
...
<div class="modal fade" id="Exit" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content border-0">
      <div class="modal-body text-center">
        ...
        <button type="button" class="btn btn-success px-4 mr-2" data-dismiss="modal">Batal</button>
  ...
  <script type="text/javascript">
    ...
    function changeValue(kode_produk) {
      ...
      document.querySelectorAll('#stok_barang').forEach(stok => {
        if (stok.getAttribute('data-kode') == kode_produk) {
          const maxQty = stok.value;
          stokBarangTersedia = maxQty;
          document.querySelector("#Iqty").setAttribute('max', maxQty);
        }
      });
    };
