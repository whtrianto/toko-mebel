...
<table class="table table-striped table-sm table-bordered dt-responsive nowrap" id="table" width="100%">
    ...
    <tbody>
        <?php
        $no = 1;
        $data_barang = mysqli_query($conn, "SELECT * FROM produk_sofa ORDER BY idproduk ASC");
        while ($d = mysqli_fetch_array($data_barang)) {
        ?>
            <tr class="text-center">
                <td class="align-middle"><?php echo $no++; ?></td>
                <td class="align-middle"><?php echo $d['kode_produk']; ?></td>
                <td class="align-middle"><img src="images/sofa/<?= $d['gambar']; ?>" width="100"></td>
                        ...
                        <form method="post" enctype="multipart/form-data">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <img src="images/sofa/<?= $d['gambar']; ?>" width="100"> <br>
...
if (isset($_POST['TambahProduk'])) {
    ...
    $cekkode = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM produk_sofa WHERE kode_produk='$kodeproduk'"));
    if ($cekkode > 0) {
        echo '<script>alert("Maaf! kode produk sudah ada");history.go(-1);</script>';
    } else {
        $InputProduk = mysqli_query($conn, "INSERT INTO produk_sofa (kode_produk,gambar,nama_produk,harga_jual,stok)
...
if (isset($_POST['SimpanEdit'])) {
    ...
    $CariProduk = mysqli_query($conn, "SELECT * FROM produk_sofa WHERE kode_produk='$kodeproduk1' and idproduk!='$idproduk1'");
    $HasilData = mysqli_num_rows($CariProduk);

    if ($HasilData > 0) {
        echo '<script>alert("Maaf! kode produk sudah ada");history.go(-1);</script>';
    } else {
        $cekDataUpdate =  mysqli_query($conn, "UPDATE produk_sofa SET kode_produk='$kodeproduk1',
...
function upload()
{
    ...
    move_uploaded_file($tmpName, 'images/sofa/' . $namaFileBaru);
    return $namaFileBaru;
}

if (!empty($_GET['hapus'])) {
    $idproduk1 = $_GET['hapus'];
    $hapus_data = mysqli_query($conn, "DELETE FROM produk_sofa WHERE idproduk='$idproduk1'");
    if ($hapus_data) {
        echo '<script>history.go(-1);</script>';
    } else {
...
<div class="modal fade" id="TambahProduk" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content border-0">
            ...
            $sql = mysqli_query($conn, "select max(kode_produk) as maxID from produk_sofa");
